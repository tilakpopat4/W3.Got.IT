...
## Entry 22 — bounded live acquisition and capture health
**What:** Hardened live Npcap acquisition for continuous use. Idle flows now
expire after 60 seconds, active flows are capped at 5,000, and the dashboard
reports packets observed, active flows, expired/evicted flows, capture start
time, and the last packet timestamp.
**Files:** `src/live_capture.py`, `dashboard/index.html`,
`dashboard/app.js`, `docs/stage8_basic_dashboard.md`
**Safety:** acquisition remains passive/read-only; no packets are transmitted,
blocked, or modified.

---

## Entry 23 — offline architecture boundary
**What:** Documented and adopted an explicit separation between saved-PCAP/
Parquet replay and live Npcap acquisition. Future live changes must remain in
the live adapter or dashboard surface and must not alter offline parsers,
replay ordering, model artifacts, or report formats.

---

## Entry 24 — friendly live-interface discovery
**What:** Added Windows adapter descriptions and status to live-interface
discovery. The dashboard now displays active Wi-Fi/Ethernet adapters while
preserving the raw Npcap device path used by Scapy.
**Files:** `src/dashboard_api.py`, `dashboard/index.html`,
`dashboard/app.js`, `docs/stage8_basic_dashboard.md`

---

## Entry 25 — live website target filtering
**What:** Added an optional live-only target-IP filter for focused website
testing. The capture remains passive, counts all observed packets, analyzes
only matching flows, and reports ignored packets separately. IPv4 and IPv6
addresses are validated at the API boundary.
**Safety:** The offline PCAP/Parquet architecture was not changed.

---

## Entry 26 — live IPv6 acquisition
**What:** Added IPv6 recognition to the live-only packet adapter. Live capture
now reconstructs IPv4 and IPv6 TCP/UDP flows, recognizes ICMPv4/ICMPv6 echo
traffic, accepts IPv4/IPv6 website filters, and formats IPv6 endpoints safely
as `[address]:port`.
**Safety:** The offline PCAP parser, replay path, model artifacts, and report
formats were not changed.

---

## Entry 27 — live benign-traffic calibration
**What:** Added a live-only calibration summary endpoint for measuring generic
classifier false positives on known-benign traffic.
**Files:** `src/live_calibration.py`, `src/dashboard_api.py`,
`docs/live_benign_calibration.md`
**Behavior:** Reports probability quantiles and estimated flag rates at
candidate thresholds `0.50`, `0.75`, `0.90`, `0.95`, and `0.99`.
**Safety:** This does not retrain models or change the threshold. Offline
PCAP/Parquet parsing, replay, model artifacts, and reports remain unchanged.

---

## Entry 28 — live port-scan detection
**What:** Added a live-only rolling-window reconnaissance detector for TCP
SYN and UDP fan-out patterns.
**Files:** `src/live_capture.py`, `docs/live_port_scan_detection.md`
**Behavior:** A source is classified as `port_scan` after 20 unique
destination ports or 5 unique destination hosts within 30 seconds. Evidence
is attached to the flow decision and sets `flagged` independently of the
generic classifier threshold.
**Safety:** No probes or packets are sent. Offline PCAP/Parquet parsing,
replay, model artifacts, and reports remain unchanged.

---

## Entry 29 — live protocol-specific DDoS evidence
**What:** Added passive live evidence for SYN-flood and high-rate UDP-flood
flow patterns.
**Behavior:** SYN-flood evidence requires 20 SYNs, a low ACK ratio, and at
least 100 packets/sec. UDP-flood evidence requires 1,000 packets and 1,000
packets/sec. Evidence is attached to live decisions and remains separate
from destination-level DoS/DDoS aggregation.
**Safety:** No traffic is generated or blocked. Offline architecture remains
unchanged.

---

## Entry 30 — standardized live and dashboard alerts
**What:** Reused the Pydantic `ThreatAlert` contract for uploaded-PCAP and
live dashboard responses.
**Behavior:** Flagged flow decisions are serialized into an `alerts` array
with UTC timestamp, flow ID, threat class, confidence, severity, anomaly
fields, model, and evidence. Detailed flow rows remain unchanged.
**Safety:** Alert generation is read-only and does not transmit, block, or
inspect payload data.

---

## Entry 31 — live WebSocket alert stream
**What:** Added `/ws/live-alerts` for near-real-time live alert delivery.
**Behavior:** The stream sends each new alert once and publishes status
snapshots four times per second. The dashboard subscribes when live capture
starts; existing HTTP polling remains available as a fallback.
**Safety:** The WebSocket only reads in-memory decisions and never transmits
network traffic to monitored interfaces.

---

## Entry 32 — live DNS/DGA metadata evidence
**What:** Added passive live heuristics for suspicious DNS query labels.
**Behavior:** Long, high-entropy, digit-heavy labels produce `dga_dns`
evidence; very long labels or names produce `dns_tunneling` evidence.
Only lengths, entropy, ratios, and an omitted-name suffix marker are retained.
**Safety:** No DNS queries are generated, and the full query name is not
stored in alert evidence. Offline parsing remains unchanged.

---

## Entry 33 — live botnet beacon timing evidence
**What:** Added passive inter-arrival timing analysis for repeated live flows.
**Behavior:** At least six intervals with a 0.5–60 second mean and coefficient
of variation at most 0.15 produce `c2_beacon` evidence.
**Safety:** Timing is metadata-only and is not treated as proof of C2. Offline
parsing, replay, and model artifacts remain unchanged.

---

## Entry 34 — live TLS/QUIC metadata anomaly evidence
**What:** Added passive high-rate anomaly heuristics for TCP/UDP port 443.
**Behavior:** TLS-like flows require at least 80 packets at 200 packets/sec;
QUIC-like flows require at least 100 packets at 250 packets/sec. Evidence
reports transport and rates without payload inspection.
**Safety:** No decryption, fingerprint collection, blocking, or traffic
modification was added. Offline behavior remains unchanged.

---

## Entry 35 — live directional exfiltration evidence
**What:** Added passive outbound-volume heuristics for possible exfiltration.
**Behavior:** TCP/UDP flows with at least 20 packets, 1 MB outbound bytes,
and at least 85% outbound dominance produce `possible_exfiltration` evidence.
**Safety:** Only direction, volume, and rate metadata are used. Payloads and
file contents are not inspected; no traffic is blocked or modified.

---

## Entry 36 — visible real-time alert feed
**What:** Added a dedicated dashboard feed for standardized WebSocket alerts.
**Behavior:** The browser retains up to 100 newest alerts and shows threat
class, severity, reason, timestamp, flow ID, and confidence. Operators can
clear the browser feed without affecting capture.
**Safety:** This is presentation-only; capture and alert generation remain
passive and read-only.

---

## Entry 37 — consolidated project status and upgrade roadmap
**What:** Added a single document summarizing the project from its initial
data/model foundation through the current live dashboard and alert feed.
**Contents:** The document records completed stages, measured model results,
current architecture, implemented six-category live evidence, limitations,
remaining SIH evidence gaps, and a prioritized upgrade plan.
**Next priorities:** Throughput/P95 latency benchmarking, labeled evaluation
of specialist detectors, dashboard alert filtering, and final reliability
hardening.
**Safety:** The roadmap preserves passive ingest, metadata-only analysis,
no probing, no decryption, no blocking, and authorized-lab-only validation.

---

## Entry 38 — replay throughput and latency benchmark
**What:** Added `src/benchmark_replay.py` to measure incremental flow
throughput and per-flow P50/P95/P99/max latency for the existing Kalman plus
XGBoost replay path.
**Output:** The benchmark writes `reports/replay_benchmark.json` and records
rows processed, alerts, flows/sec, estimated Mbps, and latency percentiles.
**Scope:** This is an offline inference benchmark after Parquet loading. It
does not claim Npcap, disk-read, packet-line-rate, or production NIC
performance. A separate live-capture benchmark remains appropriate.
**Documentation:** Added `docs/throughput_latency_benchmark.md` with commands,
measurement definitions, and interpretation guidance.
**Recorded run:** 10,000 rows produced 1,750.26 flows/sec, estimated 3.58 Mbps,
P50 0.4446 ms, P95 0.8721 ms, P99 1.8675 ms, and 30 alerts on the development
machine.

---

## Entry 39 — live callback throughput benchmark
**What:** Added `src/benchmark_live.py` to benchmark the exact live
`LiveSession.on_packet` callback used by Npcap capture.
**Modes:** The default mode uses in-memory Scapy packets and is safe without
network access. An optional bounded `--interface` mode passively reads an
authorized Npcap interface for a fixed duration and never transmits packets.
**Measurements:** The report records callback packets/sec, flow counts,
flagged flows, ignored packets, elapsed time, and measurement scope.
**Documentation:** Added `docs/live_throughput_latency_benchmark.md`.
**Recorded safe run:** 1,000 in-memory packets produced 437.97 callback
packets/sec and 1,000 active flows. The 1,000 flagged flows are not an
accuracy measurement because the packet distribution was synthetic.
